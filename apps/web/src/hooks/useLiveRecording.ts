...
import type { PartialTranscript, TranscriptSegment, SessionCostSummary } from '@transcript/shared'
...
export interface LiveTranscriptLine {
  ...
  isFinal: boolean
  start?: number
  translation?: string    // bản dịch (nếu đã dịch xong)
  translating?: boolean   // đang chờ dịch
}
...
export function useLiveRecording(opts: UseLiveRecordingOptions = {}): UseLiveRecordingReturn {
  ...
  const [translateLang, setTranslateLang] = useState<'en' | 'fr' | 'vi' | null>(
    initialTranslateLang as 'en' | 'fr' | 'vi' | null,
  )
  const translateLangRef = useRef<'en' | 'fr' | 'vi' | null>(
    initialTranslateLang as 'en' | 'fr' | 'vi' | null,
  )
  const [sourceLang, setSourceLang] = useState<string | null>(initialSourceLang)
  const sourceLangRef = useRef<string | null>(initialSourceLang)

  function handleSetSourceLang(lang: string | null) {
  ...
  function handleSetTranslateLang(lang: 'en' | 'fr' | 'vi' | null) {
    translateLangRef.current = lang
    setTranslateLang(lang)
  }

  // Dịch 1 câu final và cập nhật line đó trong state
  function translateLine(lineId: string, text: string, lang: 'en' | 'fr' | 'vi') {
    ...
    api.translate.segment(text, lang)
      ...
      .catch((err) => {
        ...
        setLines((prev) => prev.map((l) =>
          l.id === lineId ? { ...l, translating: false } : l,
        ))
      })
  }
  ...
  function sendWS(type: string, data?: unknown) {
  ...
  async function connectWebSocket(attemptsLeft = 5): Promise<WebSocket> {
    const ws = await (async (): Promise<WebSocket> => {
    ...
    ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === 'partial_transcript') {
            const data = msg.data as { sessionId?: string } & Partial<PartialTranscript>
            if (data.sessionId && !sessionIdRef.current) {
              sessionIdRef.current = data.sessionId
              setSessionId(data.sessionId)
            }
            if (data.text) {
              const partial = data as PartialTranscript
              setLines((prev) => {
                ...
                if (last && !last.isFinal) {
                  ...
                  const updated: LiveTranscriptLine = {
                    id: last.id,
                ...
                const newLine: LiveTranscriptLine = {
                  id: lineId,
          ...
          if (msg.type === 'final_transcript') {
            const segments = msg.data as TranscriptSegment[]
          ...
          if (msg.type === 'session_end') {
            const data = msg.data as { reportId: string; sessionId: string; cost?: SessionCostSummary }
            setSessionId(data.sessionId)
            setReportId(data.reportId)
            if (data.cost) setSessionCost(data.cost)
            setStatus('done')
            cleanup()
          }
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      const finalSource: AudioNode = micSource
